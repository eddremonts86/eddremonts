...
2. **Prompt Template:** The agent synthesizes the project's `description` + `category` passing it to the Image Generation tool:
   - _Example Prompt Strategy:_ `Minimalist vector art representing [Category], using neon cyan (#00E5FF) and sakura pink (#FF2A85) gradients over an off-white background (#FBFBFD). UI/UX technology themed, sharp vector style reminiscent of anime sci-fi aesthetics. No text.`
